...
def astar(start_state, goal_state, successors):
    ...
    while open_list:
        ...
        explored_nodes.append(current_state)  # Add current node to explored nodes
...
# Plot explored nodes
pos = {node: node for node in G.nodes()}  # 节点位置
nx.draw_networkx_nodes(G, pos, nodelist=explored_nodes, node_color='blue', node_size=500)  # 绘制已探索的节点
plt.show()
