...
def dijkstra(start_state, goal_state, graph):
    open_list = []
    closed_set = set()
    ...
    while open_list:
        ...
        for neighbor in graph.neighbors(current_state):
            ...
            next_node = Node(next_state, current_node, current_node.cost + cost)
...
# nx.draw(G, pos, with_labels=True, node_size=700, node_color='lightblue')  # 绘制节点
# plt.show()
path = dijkstra(start_state, goal_state, G)
print("Dijkstra算法找到的最短路径：", path)
